...
`increment_counter`, `decrement_counter`, `record_histogram`, `set_gauge`, `get_trace`
- Attributes
...
[Example](onboarding_examples.md#creating-configuration)
...
[Example](onboarding_examples.md#specific-endpoint-for-signal)
...
[Example](onboarding_examples.md#optional-session-entropy)


## Prepare the ResourceAttributes Object
The `initialize_telemetry` function also requires a `ResourceAttributes` object. Configure this class with attributes that belong to ALL telemetry generated per end-user session (these are [resource attributes](getting_started.md#resource-attributes)). For example if user `userxyz123` starts an application instrumenting with this package then ALL telemetry generated would have user_id `userxyz123` in common, so it is appropriate to be set in this class.
...
[Example](onboarding_examples.md#resourceattributes)

### Adding more attributes
Beyond the attributes already defined in the [schema](getting_started.md#telemetry-schema), any additional keys can be added as well.

[Example](onboarding_examples.md)
...
[Example](onboarding_examples.md#initializing-telemetry)
...
[Example](onboarding_examples.md#optional-signal-streams)
...
[Example](onboarding_examples.md#logs)
...
- **histogram**: This metric should be used to create distributions of values rather than sums. Examples: request latency, response payload size. Use `record_histogram` with this type.
- **gauge**: This metric records the last value set rather than a sum or a distribution. Use it for values that are sampled at a point in time and can go up or down. Examples: queue depth, memory in use, current number of open network connections. The value must be an int or a float. Use `set_gauge` with this type.

[Example](onboarding_examples.md#metrics)
...
[Example](onboarding_examples.md#traces)
...
## Attributes
Attributes are attached to individual pieces of telemetry collected by OpenTelemetry. These are best utilized as dynamic values that can change at call time. When calling `increment_counter`, `record_histogram`, or `set_gauge` in this package they can be passed as a dictionary.

## Telemetry Schema
Documented in the [schema outline](schema-versions.md). The `ResourceAttributes` class enforces this schema so that payloads are consistent.
...
A configuration of this class using all available initialization parameters would look like [this example](onboarding_examples.md#schema)
...
[Example](onboarding_examples.md#dynamic-resource-attributes-in-the-schema)