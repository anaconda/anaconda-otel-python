...
from .utils import _random_token, _read_file, _saved_token, cached
...
def _jwt_to_token(s):
    ...
    try:
        ...
        logger.debug("Unexpected %s parsing API key: %s", type(exc), exc)
...
def anaconda_auth_token():
    ...
    try:
        ...
        logger.debug("Unexpected error retrieving token using anaconda_auth: %s", exc)
