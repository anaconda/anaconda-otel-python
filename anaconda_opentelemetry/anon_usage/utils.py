...
def _read_file(fpath, what, single_line=False):
    """
...
def _saved_token(fpath, what, must_exist=None, node_tie=False):
    """
    Implements the saved token functionality. If the specified
    file exists, and contains a token with the right format,
    return it. Otherwise, generate a new one and save it in
    ...
    if len(client_token) < TOKEN_LENGTH:
        if client_token:
            logger.debug("Regenerating %s due to short length", what)
        regenerate = True
    if node_tie:
        ...
        if regenerate or not true_node:
            ...
            logger.debug("Regenerating %s due to hostID change", what)
            regenerate = True
        elif xtra:
            logger.debug("Stripping host ID from %s file", what)
            resave = True
        else:
            logger.debug("Host ID match confirmed for %s", what)
